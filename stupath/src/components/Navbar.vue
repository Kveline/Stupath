<template>
  <div class="background-nav">
    <nav>
      <div class="text-nav">
        <div>
          <router-link tag="a" to="/">Pilih Pekerjaan</router-link>
          <router-link tag="a" to="/forum">Forum</router-link>
          <router-link tag="a" to="/tambah-forum">Tambah Forum</router-link>
        </div>
        <div>
          <span v-if="userProfile">{{ userProfile.username }}</span>

          <svg
            @click="logout()"
            style="cursor:pointer;margin-left:24px"
            fill="#00918e"
            height="20px"
